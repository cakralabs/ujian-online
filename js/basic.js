...
function show_next_question(){
	...
	// hide show next back btn
	if(qn >= 1){
		$('#prev-soal').css('visibility','visible');
		$('#last-soal').css('visibility','hidden');
	}
	if((parseInt(qn)+1) == noq){
		$('#next-soal').css('visibility','hidden');
		$('#last-soal').css('visibility','visible');
	}
	change_color(lqn);
	setIndividual_time(lqn);
	save_answer(lqn);
...
function show_back_question(){
	...
	// hide show next back btn
	if(qn < noq){
	$('#next-soal').css('visibility','visible');
	}
	if(qn == 0){
	$('#prev-soal').css('visibility','hidden');
	}
	change_color(lqn);
	setIndividual_time(lqn);
	save_answer(lqn);
