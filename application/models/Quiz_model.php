...
Class Quiz_model extends CI_Model
{
  ...
  function quiz_list($limit){
	 ...
	 if($this->input->post('search') && $logged_in['su']=='1'){
		 ...
		 // menampilkan kuis yang attribut show = 1
 ...
 function update_quiz($quid){
	 ...
	 if($this->input->post('certificate_text')){
	  ...
	  $this->db->where('quid',$quid);
	  $query=$this->db->get('quiz',$userdata);
...
//FIELD(options.qid,$qids)	 
	$query=$this->db->query("select * from options where qid in ($qids)");
 	 //$query=$this->db->query("select * from options where qid in ($qids) order by rand()");
	 return $query->result_array();
...
$noq=$quiz['noq'];	
	...
	//$query=$this->db->query("select * from qbank join category on category.cid=qbank.cid where qbank.cid='$cid' and lid='$lid' limit $noq ");	
	$questions=$query->result_array();
	foreach($questions as $qk => $question){
		$qids[]=$question['qid'];
		if(!in_array($question['category_name'],$categories)){
 ...
 function open_result($quid,$uid){
	 $result_open=$this->lang->line('open');
		$query=$this->db->query("select * from result  where result.result_status='open' and result.uid='".$uid."'"); 
	if($query->num_rows() >= '1'){
		$result=$query->row_array();
return $result['rid'];		
	}else{
	...
	if($percentage_obtained >= $quiz['pass_percentage']){
	 ...
	 $qp=$r_qids[$qno];
		...
		if($$qp_score=='1'){
		  ...
		  $query_qp="update qbank set  $crin  where qid='$qp'  ";
