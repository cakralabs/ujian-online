...
Class Qbank_model extends CI_Model
{
 ...
 function question_list_new($limit,$cid='0',$lid='0'){
	 if($this->input->post('search')){
		 $search=$this->input->post('search');
		 $this->db->or_where('qbank.qid',$search);
		 $this->db->or_like('qbank.question',$search);
	 ...
	 $this->db->join('level','level.lid=qbank.lid');
	 $this->db->limit($this->config->item('number_of_rows'),$limit);
         $order = ($cid == '0')?'DESC':'ASC';
		$this->db->order_by('qbank.qid',$order);
		$query=$this->db->get('qbank');
		return $query->result_array();
 ...
 function getJumlah($id){
	$kueri_seb = $this->db->query("SELECT * FROM qbank WHERE qid < '".$id."'");
	$kueri_ses = $this->db->query("SELECT * FROM qbank WHERE qid > '".$id."'");	
	$data = array(
		'sebelum'	=> $kueri_seb->num_rows(),
		'sesudah'	=> $kueri_ses->num_rows()
	);
