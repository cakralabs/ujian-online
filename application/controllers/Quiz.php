...
class Quiz extends CI_Controller {
	...
	public function index($limit='0')
	{
		...
		// fetching quiz list
		$result = $this->quiz_model->quiz_list($limit);
		if(count($result) == 1)
		{
			if(isset($result['0']['quid']) && $logged_in['su']==0)
			{
				redirect('quiz/quiz_detail/'.$result['0']['quid']);
			}
		}
		$data['result'] = $result;
	...
	function remove_all(){
		...
		$delete = $this->quiz_model->remove_all($data);
		if($delete){
	...
	public function quiz_detail($quid){
		...
		if($open_result != '0'){
			...
			//$this->load->view('footer',$data);
			$this->load->view('quiz_detail_new',$data);
		}
	...
	*/
	...
	function attempt($rid){		
		...
		//$this->load->view('footer',$data);
		$this->load->view('quiz_attempt_new',$data);
		}
